...
import 'package:myfatoorah_flutter/model/directpayment/MFCardInfo.dart' as CardInfo;

MFError getErrorMsg(int statusCode, String body) {
  if (statusCode == ErrorHelper
      .getValue(ErrorsEnum.CONFIG_API_KEY_ERROR)
      .code)
    return ErrorHelper.getValue(ErrorsEnum.CONFIG_API_KEY_ERROR);

  else if (statusCode == ErrorHelper
      .getValue(ErrorsEnum.CONFIG_BASE_URL_ERROR)
      .code)
    return ErrorHelper.getValue(ErrorsEnum.CONFIG_BASE_URL_ERROR);

  else if (statusCode == ErrorHelper
      .getValue(ErrorsEnum.INTERNET_CONNECTION_ERROR)
      .code)
    return ErrorHelper.getValue(ErrorsEnum.INTERNET_CONNECTION_ERROR);

  else if (statusCode == ErrorHelper
      .getValue(ErrorsEnum.SERVER_ERROR)
      .code)
    return ErrorHelper.getValue(ErrorsEnum.SERVER_ERROR);

  else if (statusCode == ErrorHelper
      .getValue(ErrorsEnum.UN_AUTHORIZED_ERROR)
      .code)
    return ErrorHelper.getValue(ErrorsEnum.UN_AUTHORIZED_ERROR);

  else if (statusCode == ErrorHelper
      .getValue(ErrorsEnum.NOT_FOUND_ERROR)
      .code)
...
String validateCardInfo(CardInfo.Card card) {
  if (card.number.isEmpty || card.number.length > 19)
    ...
    return ErrorHelper
        .getValue(ErrorsEnum.INVALID_CARD_EXPIRY_MONTH_ERROR)
        .message;
  else if (card.expiryYear.isEmpty || card.expiryYear.length > 2)
    return ErrorHelper
        .getValue(ErrorsEnum.INVALID_CARD_EXPIRY_YEAR_ERROR)
        .message;
  else if (card.securityCode.isEmpty || card.securityCode.length < 3 ||
      card.securityCode.length > 4)
    return ErrorHelper
        .getValue(ErrorsEnum.INVALID_CARD_SECURITY_CODE_ERROR)
        .message;
  else if (card.cardHolderName.isEmpty ||
      new RegExp("^[a-zA-Z0-9]*\$").hasMatch(
          card.cardHolderName.replaceAll(" ", "")) == false)
    return ErrorHelper
        .getValue(ErrorsEnum.INVALID_CARD_HOLDER_NAME_ERROR)
        .message;
  return "";
}